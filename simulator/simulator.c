...
instruction_t* decode_instructions(unsigned int* bytes, unsigned int num_instructions)
{
  ...
  for (index  = 0; index < num_instructions; index++) {
    decode.opcode =  bytes[index] >> 27;
    decode.first_register = (bytes[index] >> 22) & 31;
    decode.second_register = (bytes[index] >> 17) & 31;
    decode.immediate = bytes[index] & 65535;
...
unsigned int execute_instruction(unsigned int program_counter, instruction_t* instructions, int* registers, unsigned char* memory)
{
  ...
  case movl_reg_reg:
    ...
    break;
  case movl_deref_reg:
    registers[instr->second_register] = *(unsigned int*)(memory + registers[instr->first_register] + instr->immediate);
  ...
  case cmpl: {
    ...
    //       > (unsigned int) registers[instr->second_register]) {
    //         registers[16] |= 0x1;
    //       }
    ...
    // if (registers[instr->second_register] - registers[instr->first_register] > registers[instr->second_register]
    //  && registers[instr->second_register] - registers[instr->first_register] < 0) {
    //   registers[16] |= 0x800;
    // }
    ...
    // if ((!sA && !sB && sResult) || (!sA && sB == 1 && !sResult) || (!sA && sB && sResult) || (sA && sB&& sResult)) {
